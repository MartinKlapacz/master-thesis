...
The following code displays a simplified version the of protobuf message for the \code{SpineInfiltrationMsg}procedure block.
\lstinputlisting[
    language=protobuf3,
    style=protobuf,
...
In order to get the total number of injections, the system would need to peek into the nested messages,\code{CervicalSpineMsg}\code{ThoracicSpineMsg} and\code{LumbarSpineMsg} count the total number of \code{LATERALITY\_LEFT} and \code{LATERALITY\_RIGHT} values in all localization fields.
