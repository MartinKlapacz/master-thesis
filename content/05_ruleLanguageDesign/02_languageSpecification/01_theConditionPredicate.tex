...
\code{singleBillingPosition} is a special condition type that if set to true only evaluates to \code{true} if it is the only code in the derivation result.
...
%\code{\{singleBillingPositionIfPractitionerIsDoctor : true\} = \code{true}} \leftrightarrow
\todo{finish this}
\[
...
that if set to true only evaluates to \code{true} if it is the code in the derivation result.
